...
import {
  ...
  RefreshCw, ChevronDown, ChevronUp, ArrowUp, ArrowDown, ArrowUpDown, Plus, Maximize2, Minimize2,
  FileSpreadsheet, FileText, File, ExternalLink, Hash, Users, Wallet, Target, Percent, TrendingUp, TrendingDown
} from 'lucide-react';
import { INITIAL_TRANSAKSI, STATUS_TRANSAKSI_OPTIONS, PROGRAM_TRANSAKSI } from '../utils/crmTransaksiStore';
...
const RUTIN_PAGE_SIZES = [10, 25, 50];
const fmtRutin = (n) => new Intl.NumberFormat('id-ID', { minimumFractionDigits: 0, maximumFractionDigits: 0 }).format(n || 0);

// Rekap donatur rutin (bulanan/mingguan) — dulu halaman terpisah "List Transaksi Rutin",
// sekarang jadi mode di dalam List Transaksi yang sama (lihat toggle "Rutin" di filters-row).
const RutinView = () => {
            ...
            <tbody>
              ...
              {paged.map(r => (
                <tr key={r.id}>
                  ...
                  <td>{r.timCrm}</td>
                  <td style={{ textAlign: 'right', fontWeight: 600 }}>{fmtRutin(r.total)}</td>
                  {transaksiMode === 'Bulan' && r.monthly.map((v, i) => (
                    <td key={MONTHS[i].key} style={{ textAlign: 'right', color: v === 0 ? '#cbd5e1' : undefined }}>{fmtRutin(v)}</td>
                  ))}
                </tr>
              ))}
            </tbody>
            {sorted.length > 0 && (
              <tfoot>
                <tr style={{ fontWeight: 700 }}>
                  ...
                  <td style={{ textAlign: 'right' }}>{fmtRutin(totals.total)}</td>
                  {transaksiMode === 'Bulan' && totals.monthly.map((v, i) => (
                    <td key={MONTHS[i].key} style={{ textAlign: 'right' }}>{fmtRutin(v)}</td>
                  ))}
                </tr>
              </tfoot>
            )}
        ...
        <div className="pagination-bar">
          ...
          <div className="pagination-controls">
            <select className="pagination-size-select" value={pageSize} onChange={e => { setPageSize(Number(e.target.value)); setPage(1); }}>
              {RUTIN_PAGE_SIZES.map(s => <option key={s} value={s}>{s} / halaman</option>)}
            </select>
...
const ListTransaksi = () => {
  ...
  const [isRutinMode, setIsRutinMode] = useState(false);
    ...
    <div className={isFullscreen ? 'content-area content-area-fullscreen' : 'content-area'}>
      <div className="page-header">
        <div className="page-title">
          ...
          <p>{isRutinMode
            ? 'Rekap donatur dengan kesediaan donasi rutin (bulanan/mingguan) beserta riwayat transaksinya'
            : 'Daftar transaksi donasi beserta status approval dan sinkronisasi ke FINS'}</p>
        </div>
      ...
      <div className="filters-row" style={{ flexWrap: 'wrap', rowGap: '10px' }}>
        <div className="filters-left" style={{ flexWrap: 'wrap', rowGap: '10px' }}>
          <label className="toggle-switch" title="Tampilkan rekap donatur rutin">
            <input type="checkbox" checked={isRutinMode} onChange={e => setIsRutinMode(e.target.checked)} />
            <span className="toggle-slider" />
          </label>
          <span style={{ fontSize: '0.8rem', color: '#64748b', fontWeight: 600 }}>Rutin</span>
        </div>
      </div>
